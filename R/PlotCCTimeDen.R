...
#' PlotCCTimeDen(example_sce$CCTime, example_sce$CCStage, "sample")
NULL
...
PlotCCTimeDen <- function(theta.v, color_var.v, color_name, palette.v = NULL, fig.title = NULL, type = c("linear", "circular"), bw = 30,  weighted = FALSE, line.size = 0.5, line.alpha = 0.5, ...) {
	...
	if (is.null(palette.v)) {
		...
		if (nlevels(color_var.v) > length(palette.v)) warning("The number of colors in palette.v doesn't match the number of levels in color_var.v. ")
	}
