...
#' example_sce2 <- inferCCStage(example_sce, gname.type = "ENSEMBL", species = "mouse", batch = "sample")
#' example_sce3 <- inferCCStage(example_sce, gname.type = "ENSEMBL", species = "mouse", batch = example_sce$sample)
...
.CCStage <- function(data.m, batch.v = NULL, cycleGene.l = NULL,  corThres = 0.2, tolerance = 0.3) {
	if (is.null(batch.v)) batch.v <- 	rep(1, ncol(data.m))
	if ((ncol(data.m) != length(batch.v)) | any(is.na(batch.v))) stop("Something is wrong with batch argument. Refer to manual.")
	

	cycleGene.l <- lapply(cycleGene.l, intersect, rownames(data.m))
	allgenes.v <- purrr::reduce(cycleGene.l, union)
	if (sum(allgenes.v %in% rownames(data.m)) < 30)  stop("Less than 30 cell cycle gene found. Not enough infomation to assgin the 5 stages.")
	...
	for (b in unique(batch.v)) {
		...
		score.m <- t(scale(t(scale(do.call(cbind, lapply(seq_along(cycleGene.l), function(i) {
			...
			if (sum(cor.v > corThres) < 3) warning(paste0("Batch ", b, " phase ", names(cycleGene.l)[i], " too few genes (<3)."))
...
.inferCCStage <- function(data.m, cycleGene.l = NULL, gname = NULL, gname.type = c("ENSEMBL", "SYMBOL"), species = c("mouse", "human"), AnnotationDb = NULL, ...){
	...
	if (is.null(cycleGene.l)) {
		if (species == "mouse") {
			...
			if (gname.type == "ENSEMBL") rownames(data.m) <- .getSYMBOL(gname, species = "human", AnnotationDb = AnnotationDb)
		}
...
setMethod("inferCCStage", "SummarizedExperiment", function(x, ..., exprs_values = "logcounts", batch.v = NULL) {
	if (!is.null(batch.v)) {
		if ((length(batch.v) == 1) & all(batch.v %in% names(colData(x)))) batch.v <- colData(x)[, batch.v]
	}
