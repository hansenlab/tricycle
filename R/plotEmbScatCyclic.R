...
#' 
.plotEmbScatCyclic <- function(emb.m, color.value, color_by, facet_var = NULL, fig.title = NULL, point.size = 2.1, point.alpha = 0.6, x_lab = NULL,y_lab = NULL,
														 hue.colors = c("#2E22EA","#9E3DFB","#F86BE2","#FCCE7B","#C4E416","#4BBA0F","#447D87","#2C24E9"), hue.n = 500, plot.legend = FALSE) {
	if (is.null(fig.title)) fig.title <- paste0("(n=", nrow(emb.m), ")")
	
	if (is.null(x_lab)) x_lab <- colnames(emb.m)[1]
	if (is.null(y_lab)) x_lab <- colnames(emb.m)[2]
	...
	scat.p <- ggplot(tmp.df, aes(x = x, y = y, color = color)) +
		...
		scale_color_gradientn(name = color_by, limits = range(0, 2 * pi), 
...
{
	if (length(dim) != 2) stop("The function can only plot 2 dims at this time. Change dim argument.")
	emb.m <- reducedDim(sce.o, dimred)[, dim]
	color.value <- colData(sce.o)[[color_by]]
	if (!is.null(facet_by)) {
		if (!(facet_by %in% names(colData(sce.o)))) stop("facet_by variable does not exist in colData(sce.o).")
...
#' @import ggplot2
#' @export
cyclic_legend <- function(hue.colors = c("#2E22EA","#9E3DFB","#F86BE2","#FCCE7B","#C4E416","#4BBA0F","#447D87","#2C24E9"),
	...
	legend.p <- ggplot(hues.df) + 
		geom_rect(
			aes(ymin = y.inner, ymax = y.outer, xmin = theta - 0.001,xmax = theta + 0.001,color = colors,fill = colors), alpha = alpha,
		...
		theme_void() + 
		ylim(c(0, ymax)) + 
		geom_text(data = hue_text.df,
							aes(x = theta, y = y.text, label = label, hjust = hjust), size = text.size ) +
